# Ten-step-learning source entry

This directory is the authoritative source unit for the Ten-step-learning portable Skill suite. It lives inside the parent `project/` Git repository; product facts here override remembered conversation state and the sibling Agent Context repository.
...
3. Inspect the target Skill directory and check Git status from the parent repository root before editing.
...
- Store each real skill directly at `<this-source-root>/<skill-name>/SKILL.md`; the folder name must equal the frontmatter `name`.
...
- Product facts, accepted specifications, plans, decisions, and validation evidence belong in this source unit, not in the parent maps or sibling Agent Context repository.
- Keep generated Codex Plugin output outside the parent Git repository under `../../codex-plugin/`.

## Validation and completion

Run from this source root (`project/ten-step/`):
